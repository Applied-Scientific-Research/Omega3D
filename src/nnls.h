...
protected:
  ...
  Index _argmax_Z(const RowVectorType &v) {
    ...
    //std::cout << "in _argmax_Z " << (_Np+1) << " " << _A.cols() << std::endl;
  ...
  Scalar _max_Z(const RowVectorType &v) {
    ...
    //std::cout << "in _max_Z m=" << m << std::endl;
    return m;
  }
...
bool NNLS<MatrixType>::solve(const ColVectorType &b, Heuristic heuristic)
{
  ...
  while (true)
  {
    ...
    //std::cout << "before check _Np=" << _Np << " _A.cols=" << _A.cols() << std::endl;

    // Check if system is solved:
    // somehow this _max_Z can be negative!
    if ((_A.cols()==_Np) || ((_max_Z(_w)-_epsilon)<0)) { return true; }

    // if we got this far with _A.cols() > 6 and _Np == _A.cols()-1 then we're in trouble
    //if ((_A.cols() > 6) && (_Np == _A.cols()-1)) { return true; }

    //std::cout << "before heuristic _Np=" << _Np << " _A.cols=" << _A.cols() << std::endl;
    switch (heuristic) {
...
void NNLS<MatrixType>::_addToP(Index idx)
{
  ...
  //std::cout << "Perform rank-1 update " << (_Np-1) << std::endl;
...
void NNLS<MatrixType>::_remFromP(Index idx)
{
  ...
  //std::cout << "Update QR decomposition " << idx << " to " << _Np << std::endl;
