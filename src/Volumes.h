/*
 * Volumes.h - Specialized class for volumes in 3D
 *
...
*/
    ...
    // compute all basis vectors and element volumes
    compute_bases(nb);
...
/*
    if (ps[0]) {
      ...
      ElementPacket<float> pts = represent_as_particles(0.0);

      // now compute impulse of those
      for (size_t i=0; i<pts.nelems; ++i) {
        imp[0] -= pts.val[i] * pts.x[2*i+0];
