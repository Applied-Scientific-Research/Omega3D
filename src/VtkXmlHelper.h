...
void write_vtu_points(Points<S> const& pts, const size_t file_idx, const size_t frameno) {
  ...
  printer.PushText( " " );
  for (size_t i=0; i<pts.get_n(); ++i) { std::fprintf(fp, "%g %g %g ", x[0][i], x[1][i], x[2][i]); }
  ...
  for (size_t i=0; i<pts.get_n(); ++i) {
    std::fprintf(fp, "%g %g %g ", u[0][i], u[1][i], u[2][i]);
  }
  printer.CloseElement();	// DataArray
  ...
  if (has_strengths) {
    ...
    // end the element header and write the data
    printer.PushText( " " );
    const std::array<Vector<S>,Dimensions>& s = pts.get_str();
    for (size_t i=0; i<pts.get_n(); ++i) {
      std::fprintf(fp, "%g %g %g ", s[0][i], s[1][i], s[2][i]);
    }
    // write binary version
