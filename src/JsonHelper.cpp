...
void read_json (Simulation& sim,
  ...
  if (j.count("version") == 1) {
    ...
    if (params.find("Omega2D") != params.end()) {
      std::cout << "  is an Omega2D file" << std::endl;
      throw "Input file is not compatible.";
    }
    if (params.find("Omega3D") != params.end()) {
      int o3dv = params["Omega3D"];
      std::cout << "  is an Omega3D file, version " << o3dv << std::endl;
    }
    if (params.find("OmegaFlow") != params.end()) {
      int of2dv = params["OmegaFlow"];
      std::cout << "  is an OmegaFlow file, version " << of2dv << std::endl;
    }
...
void write_json(Simulation& sim,
  ...
  if (desc.empty()) {
    j["description"] = "Simulation created by Omega3D";
  } else {
    j["description"] = desc;
  }
  ...
  j["drawparams"] = { {"backgroundColor", {rp.clear_color[0], rp.clear_color[1], rp.clear_color[2], rp.clear_color[3]} },
                      ...
                      {"viewPoint", {rp.vcx, rp.vcy, rp.vcz} },
                      {"viewScale", rp.vsize},
